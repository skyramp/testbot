...
import { exec, debug, withGroup, withRetry, secondsToMilliseconds } from './utils'
    ...
    if (config.skyrampMcpSource === 'github') {
      ...
      try {
        await withRetry(async () => {
          ...
          fs.rmSync(mcpPkgDir, { recursive: true, force: true })
        ...
        // Ensure PAT token embedded in .git/config doesn't persist on final failure
        fs.rmSync(mcpPkgDir, { recursive: true, force: true })
        throw err
      }
      ...
      await withRetry(
        () => exec('npm', ['install', pkg], { cwd: mcpInstallDir, timeout: NPM_INSTALL_TIMEOUT_MS }),
